...
res_fc = res %>%
    group_by(level, annot) %>%
    summarise(
        mean_hit = mean(mean[hit_or_match == "hit"], na.rm = TRUE),
        mean_match = mean(mean[hit_or_match == "match"], na.rm = TRUE),
        lower_hit = mean(lower[hit_or_match == "hit"], na.rm = TRUE),
        upper_hit = mean(upper[hit_or_match == "hit"], na.rm = TRUE),
        lower_match = mean(lower[hit_or_match == "match"], na.rm = TRUE),
        upper_match = mean(upper[hit_or_match == "match"], na.rm = TRUE),
        .groups = "drop"
    ) %>%
    mutate(
        fold_change = mean_hit / mean_match
    ) %>% 
    filter(annot %in% c("ENCODE_PLS", "ENCODE_dELS", "ENCODE_pELS", "FANTOM_enhancer", "FANTOM_promoter")) %>% 
    rowwise() %>%
    mutate(
...
ggplot(res_fc, aes(x=annot, y=fold_change, fill=level)) + 
    geom_col(position = position_dodge(width = 0.8), width = 0.7) + 
    theme_classic() +
    scale_fill_manual(
        values=annot.class.palette
    ...
    geom_errorbar(
        data = res_fc,
        aes(ymin = lower_hit, ymax = upper_hit),
        position = position_dodge(width = 0.8),
        width = 0.2
    )
